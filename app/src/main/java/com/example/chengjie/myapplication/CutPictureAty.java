...
import static com.yalantis.euclid.library.EuclidActivity.back;
        ...
        switch (v.getId()) {
            case R.id.save:
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        String url = "http://49.140.61.67:8080/Server/upLoadFile?filename=" + name + ".jpg";
                        FileImageUpload.uploadFile(file, url);
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Picasso.with(CutPictureAty.this).load(file).into(back);
                                TeaInfoActivity.profile.withIcon(bitmap);
                                TeaInfoActivity.headerResult.updateProfile(TeaInfoActivity.profile);
                            }
                        });
                    }
                }).start();
